...
class TestAutogenCourtsJson(unittest.TestCase):
    ...
    def test_autogen_courts_json(self):
        ...
        assert '"identifier_iri": "http://www.tribunals.gov.uk/ImmigrationAsylum/"' in self.raw_json

    def test_has_sensible_ontology_iri(self):
        assert (
            '"ontology_iri": "https://caselaw.nationalarchives.gov.uk/akn/ontology/organization/ftt-transport"'
            in self.raw_json
        )


class TestCourtOutputPython(unittest.TestCase):
    uksc = courts.get_by_param(CourtParam("uksc"))
    transport = courts.get_by_code(CourtCode("FTT-Transport"))

    def test_ontology_iri(self):
        assert self.uksc.ontology_iri == "https://caselaw.nationalarchives.gov.uk/akn/ontology/organization/uksc"
        assert (
            self.transport.ontology_iri
            == "https://caselaw.nationalarchives.gov.uk/akn/ontology/organization/ftt-transport"
        )

    def test_identifier_iri(self):
        assert self.transport.identifier_iri == "http://www.tribunals.gov.uk/ImmigrationAsylum/"
        assert (
            self.uksc.identifier_iri
            == "https://webarchive.nationalarchives.gov.uk/ukgwa/20160811143636/http://www.transporttribunal.gov.uk/Aspx/Default.aspx"
        )
