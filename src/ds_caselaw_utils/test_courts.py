...
class TestCourtWithJurisdiction(unittest.TestCase):
    ...
    def test_link_and_iri_with_custom_iri(self):
        # It returns the court link
        court = mock_with_properties({"link": "court_link", "identifier_iri": "court_identifier_iri"})
        jurisdiction = mock_with_properties()
        cwj = CourtWithJurisdiction(court, jurisdiction)
        self.assertEqual(cwj.link, "court_link")
...
class TestAutogenCourtsJson(unittest.TestCase):
    def test_autogen_courts_json(self):
        ...
        datafile = datafile = pathlib.Path(__file__).parent.parent.parent.parent / "autogen/courts.json"
        with open(datafile) as f:
            raw_json = f.read()
